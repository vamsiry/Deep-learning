...
def model_function(features, targets, mode):      
    ...
    hlayers = layers.stack(features, layers.fully_connected, [20,10], activation_fn=tf.nn.relu, weights_initializer=layers.xavier_initializer(uniform=True,seed=100))
...
classifier.fit(x=x_train, y=y_train, steps=10000, batch_size=1000)
